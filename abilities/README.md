...
- `drawing/` — the unified visual output ability. It covers transparent screen
  annotations (circles, arrows, rectangles, and points) as well as short
  on-screen text, labels, and prompts. It also owns the `focus_screen_region`
  helper for edge-aware local zoom before precise drawing when full-frame
  location is uncertain.
- `writing/` — a backwards-compatible internal tool contract used by the
  unified Drawing ability. It is not a separately selectable Role ability.
...
transcript; it does not require the model to call the internal on-screen
writing tool.
