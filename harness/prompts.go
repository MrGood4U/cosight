...
每次输出至少包含一个 speak action；speak 是必需的，即使同时需要 draw 也不能省略。
 ...
 如果 recentVision 存在，它按时间从旧到新包含最近若干次成功视觉结果；latestVision 是其中最新一项。需要判断前后差异或连续状态时才比较 recentVision，普通问题优先参考 latestVision.scene，再结合 summary、objects 和 textBlocks；不要仅根据 objects 的数量推断场景。
...
输出格式必须是：
{"scene":"...","summary":"...","objects":[{"objectId":"obj_1","label":"...","sub_label":"...","bbox_2d":[x_min,y_min,x_max,y_max],"confidence":0,"attributes":{}}],"textBlocks":[{"text_content":"...","bbox_2d":[x_min,y_min,x_max,y_max],"confidence":0}]}
...
8. 只返回完成任务所需的最少信息；禁止输出推理过程、解释、冗余场景描述、重复目标、无关属性或长篇 summary。
9. summary 只能是一句很短的关键状态；没有关键状态时返回空字符串。scene 无法可靠判断时返回空字符串。objects 和 textBlocks 没有内容时必须返回空数组 []，不要省略字段，也不要返回 null。
10. 没有补充属性时 attributes 必须返回空对象 {}。即使没有识别到任何目标，也必须返回完整 JSON 对象，例如 {"scene":"","summary":"","objects":[],"textBlocks":[]}。`
}

func seeUserPrompt() string {
	return "分析当前共享屏幕：先用 1 到 2 句简洁中文概括整幅画面的 scene，再提取与当前交互和用户可能问题相关的 UI 对象、文字和关键状态，最多返回 8 个最相关的 objects。优先前景应用窗口、可交互控件、关键文字和明显目标，忽略背景、装饰、重复图标和无关小物体。使用 Qwen-VL 官方 [x_min,y_min,x_max,y_max] 格式输出 JSON。禁止冗余描述、推理过程和无关字段；超过 8 个时只保留最相关的目标；无法可靠判断场景时 scene 返回空字符串；没有对象或文字时分别返回 objects:[] 和 textBlocks:[]，不要返回 null 或省略字段。"
}

func roleLanguage(role map[string]any) string { return stringValue(role["language"], "auto") }
