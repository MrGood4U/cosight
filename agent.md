...
- 角色名称、角色 ID、角色专属行为、提示词、默认参数和业务策略必须来自配置/数据层；官方示例角色统一维护在 `data/sample-roles.json`，不能在 `electron/`、`src/`、`harness/` 或其他运行时代码中按具体角色做特殊判断或复制一份默认值。
- 新增或修改角色行为时，优先修改角色数据并让通用运行时代码读取该字段。禁止通过“如果 roleId 等于某值”“如果角色名称包含某字符串”或类似分支实现角色特例。
- 产品可调的时间、数量、阈值、模型名、音色、提示词等都属于配置，不得散落为 magic number 或字符串。默认值只能在对应配置 schema/数据文件中定义一次；规范化函数只负责校验边界和兼容缺省值，不能覆盖具体角色的数据。
- 允许硬编码的内容仅限协议字段、文件格式版本、能力 ID、最低/最高安全边界等技术常量；这些常量必须使用有意义的命名，并且不能承载某个角色或产品策略。
- 每次代码评审都要检查新增的角色名、角色 ID、业务提示词和业务阈值是否误写进运行时代码；相关回归测试应验证运行时读取官方数据，并防止同一配置在代码中出现第二份。
